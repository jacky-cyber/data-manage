...
select
  ...
  t2.总销 as 上期总销,
  t3.总销 as 上年总销,
  sum (t1.内销) over (partition by t1.年,t1.二级部门名称 order by t1.期 ) as 累计内销,
  sum (t1.外销) over (partition by t1.年,t1.二级部门名称 order by t1.期 ) as 累计外销,
  sum (t1.总销) over (partition by t1.年,t1.二级部门名称 order by t1.期 ) as 累计总销,
  sum (t3.内销) over (partition by t3.年,t3.二级部门名称 order by t3.期 ) as 上年累计内销,
  sum (t3.外销) over (partition by t3.年,t3.二级部门名称 order by t3.期 ) as 上年累计外销,
  sum (t3.总销) over (partition by t3.年,t3.二级部门名称 order by t3.期 ) as 上年累计总销
from
  w1 t1
  left join w1 t2 on t1.二级部门名称 = t2.二级部门名称 and ((t1.年 = t2.年 and t1.期 = t2.期 + 1) or (t2.年 = 2014 and t2.期 = 52 and t1.年 = 2015 and t1.期 = 1))
