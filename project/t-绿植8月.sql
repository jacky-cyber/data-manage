...
from
  ...
  left join 产品成本 t2 on t1.产品长代码 = t2.产品长代码
where
  仓库按名称.一级部门名称 = '绿植零批运营部'
  and (select 范围 from w日期范围) @> (t1.会计期间 || '.01')::date
  and not (t1.仓库 in ('昆山虹越（农博园）','昆山虹越（花木城）','千灯店卖场') and (t1.会计期间 || '.01')::date >= '2015-01-01')
group by
  1
), w客流 as (
select
...
select
  generate_series(8,8,1) as 月
)
select
  *
from
  w编号
  full join w销售_2015 using (月)
  full join w损耗_2015 using (月)
  full join w盘损 using (月)
  full join w库存 using (月)
  full join w客流 using (月)
order by
  1;
